...
class ViewModel: ObservableObject {
    ...
    init() {
        if let user = DataModel.getLocalUserFromAppStorage() {
            ...
            localUser = User(username: "default account", diaries: exampleDiaries, avatar: Avatar(color: .orange, mood: .neutral, accessory: nil), extraDinoPoints: 0, averageDriving: 1, averagePhone: 195, averageAppliances: 1, averageEating: 1800, averageShower: 10)
        }
    }
